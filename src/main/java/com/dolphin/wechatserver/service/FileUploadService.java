...
public class FileUploadService {
    ...
    // 失败码
    private static Integer FAIL_CODE_NO_FILE = 1; // 没有文件
    ...
    private String createRandomFileSimplePath(String originalFileName) {
        ...
        return UUID.randomUUID().toString().replaceAll("-", "") + (haveSuffix ? "." + suffix : "");
    }
    ...
    public ResponseData uploadFile(HttpServletRequest request, String basicFilePath) {
        ...
        if (ObjectUtils.isEmpty(multipartFileList)) {
            return ResponseData.createFailResponseData(FAIL_CODE_NO_FILE, "没有文件");
        }
        // 获取要上传的文件的文件流
        ...
        return ResponseData.createSuccessResponseData("uploadFileNameList", resultList);
    }
...
//            return ResponseData.createFailResponseData("batchDeleteFileInfo", false, "文件名, 无法删除", "file_name_is_empty");
//        // 过滤掉空字符串
